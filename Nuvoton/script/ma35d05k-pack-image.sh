...
IMAGE_BASENAME="openwrt-ma35d05k"
...
is_secure_boot()
{
	if grep -Eq "^CONFIG_MA35D05K_SECURE_BOOT=y$" ${CONFIG_FILE}; then
		echo "yes"
	else
		echo "no"
	fi
...
uboot_cmd() {
	cp ${TOPDIR}/Nuvoton/uboot_env/ma35d05k-uboot-env.txt ${STAGING_DIR_IMAGE}/uboot-env.txt
...
build_fip() {
	if [ $IS_SECURE == "yes" ]; then
		...
		cat ${TOPDIR}/Nuvoton/nuwriter/en_fip.json | \
			jq -r ".header.aeskey = \"${CONFIG_MA35D05K_AES_KEY}\"" | \
			jq -r ".header.ecdsakey = \"${CONFIG_MA35D05K_ECDSA_KEY}\"" \
...
IMAGE_CMD-spinand() {
	...
	${STAGING_DIR_HOST}/bin/ubinize ${UBINIZE_ARGS} -o ${STAGING_DIR_IMAGE}/uboot-env.ubi-spinand ${STAGING_DIR_IMAGE}/ma35d05k-uEnv-spinand-ubi.cfg
...
IMAGE_CMD_sdcard() {
	...
	if [ -n "${CONFIG_TARGET_SDCARD_DATA_PARTSIZE}" ] && [ "${CONFIG_TARGET_SDCARD_DATA_PARTSIZE}" -gt 0 ]; then
		...
		# Use the real p4 offset recorded by gen_ma35d05k_sdcard_img.sh (ptgen
